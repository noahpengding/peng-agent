    ...
    def analyze_document(self, document: bytes) -> str:
        ...
        if result.tables:
            for table in result.tables:
                ...
                for row in grid:
                    table_text += " | ".join(cell.ljust(col_widths[i]) for i, cell in enumerate(row)) + "\n"
                
                document_text += table_text + "\n"
        for page in result.pages:
            if page.lines:
                sorted_lines = sorted(page.lines, key=lambda line: line.polygon[1] if line.polygon else 0)
