...
- **Temporary Chat Scope:** Temporary chat suppresses chat-table persistence and Datadog LLM Observability export, but model providers and enabled tools still process request content. Uploaded files are stored before chat execution, and lower-level debug logs in agent/model adapters can contain prompts or responses when debug logging is enabled.
- **Temporary Chat Continuity:** Conversation context is reconstructed from persisted numeric chat IDs. Because a temporary turn has no persisted ID, it is not available as context for a later turn; temporary mode currently behaves as ephemeral single-turn execution plus any explicitly selected persisted memories.
